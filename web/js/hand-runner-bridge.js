...
const HandRunner = (() => {
  ...
  // Engage only at the EXTREME edges; release as soon as the hand comes back
  // toward center. Dropping the hand low (REST_Y) stops everything instantly.
  const JOY_L_ON = 0.24, JOY_L_OFF = 0.45;
  const JOY_R_ON = 0.76, JOY_R_OFF = 0.55;
  const JUMP_Y = 0.42, JUMP_Y_DIAG = 0.55, REST_Y = 0.70;
  ...
  function onHand(results) {
    ...
    if (JOYSTICK_GAMES.has(gameId)) {
      ...
      // Hand resting low = stop immediately (drop your hand to halt).
      if (py > REST_Y) {
        releaseKeys();
        if (hudMove) hudMove.textContent = "REST";
        return;
      }
      if (joy.left) {
      ...
      // Jump is independent of direction — combines into diagonal holds.
      // When a direction is held, jump engages at a lower raise (harder to
      // lift the hand high while stretched sideways). Hysteresis on release.
      const diagonal = joy.left || joy.right;
      const jumpOn = diagonal ? JUMP_Y_DIAG : JUMP_Y;
      const jumpOff = diagonal ? JUMP_Y_DIAG + 0.08 : JUMP_Y + 0.08;
      const wantJump = joy.jump ? (py < jumpOff) : (py < jumpOn);
      if (wantJump !== joy.jump) { sendKey("jump", wantJump ? "down" : "up"); joy.jump = wantJump; }
